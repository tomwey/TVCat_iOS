...
- (NSString *)timeAgo
{
    ...
    if ( [self isLastWeek:secondsSince] ) {
        return [self formatAsLastWeek2];
    }
    
    if ( [self isLastYear:secondsSince] ) {
        return [self formatAsDate];
    }
    
    return [self formatAsOther2];
}
